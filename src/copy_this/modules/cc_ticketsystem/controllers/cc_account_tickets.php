...
class cc_account_tickets extends Account {
	...
	*/
	...
	public function newTicket() {
		...
		$sSubject = trim( (string) oxRegistry::getConfig()->getRequestParameter('ticketsubject', true));
		$sText = trim( (string) oxRegistry::getConfig()->getRequestParameter('tickettext', true));
		...
		if (!$sUserId) {
			...
			return oxRegistry::getUtils()->redirect('?cl=' . __class__, true, 302);
		}
		if (!$sShopId) {
		...
		if ($sSubject == '' || $sText == '') {
			...
			return oxRegistry::getUtils()->redirect('?cl=' . __class__ . '&ticket=new', true, 302);
		}
		...
		return oxRegistry::getUtils()->redirect( '?cl=' . __class__ . '&ticket=' . $oTicket->getId(), true, 302 );
	}
	...
	*/
	...
	public function updateTicket() {
		$oxConfig = $this->getConfig();
		$sText = trim( (string) oxRegistry::getConfig()->getRequestParameter('tickettext', true));
		$sTicketId = trim( (string) oxRegistry::getConfig()->getRequestParameter('tid', true));
		$sTimestamp = date('Y-m-d H:i:s');
		$sShopId = $oxConfig->getShopId();
		$sUserId = oxRegistry::getSession()->getVariable('usr');
		if (!$sUserId) {
		...
		if ($sText == '') {
			oxRegistry::get("oxUtilsView")->addErrorToDisplay('CC_TICKETSYSTEM_ERROR_EMPTY');
			return oxRegistry::getUtils()->redirect('?cl=' . __class__ . '&update=true&ticket=' . $sTicketId, true, 302);
		}
		$oTicket = oxNew('cc_ticket');
